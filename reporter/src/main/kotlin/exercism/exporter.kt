...
fun exportTestPlanResults(name: String, cases: List<TestItem>, dir: File) {
    val json = TestSuit(cases).toJsonString()
...
fun TestSuit.toJsonString(prettyPrint: Boolean = true): String {
    ...
    return moshi
        .adapter(TestSuit::class.java)
        .let { if (prettyPrint) it.indent("  ") else it }
        .toJson(this)
}
